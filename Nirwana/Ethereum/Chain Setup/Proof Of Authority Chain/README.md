# Setup Private Blockchain with Parity 
...
"gasLimitBoundDivisor" which determines gas limit adjustment has the usual Ethereum value "stepDuration" is set to at least 5 second block times "validators" is empty right now since we are yet to create our authority accounts
...
Now that a bare bones chain specification is complete the two nodes can be set up. Parity stores accounts for each chain with a different genesis hash in a separate folder, so in order to create the accounts in the correct one we will need to run with the --chain option. Normally the two nodes would be started on separate machines, however since we are using the same one we will need to resolve some possible collisions:
...
--ws-port is the port used by the wallet to talk to the node We will also want to expose all RPC apis to make interacting with the nodes easier 
...
Alternative config files can be generated here.
...
In order to ensure the nodes are connected to each other we will need to know their enode addresses and inform the other node about it. There are two ways to obtain the enode:
...
To deploy Parity on multiple machines you may find the docker builds useful.

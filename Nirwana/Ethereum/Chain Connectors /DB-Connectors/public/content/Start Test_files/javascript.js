...
function isDate(value) {
	...
	if (isNaN(d) || d>31) return false;
	if (isNaN(m) || m>12) return false;
	if (isNaN(y) || y>3000) return false;
...
function showCalendar(year, month, day, pattern, formName, formProperty, event, startYear, endYear) {
	...
	if(document.all) {
		...
		// Netspace 4
		document.slcalcod.left = e.pageX+10+'px';
		document.slcalcod.top = e.pageY+10+'px';
...
function cal_place(event) {
	var calendrier = document.getElementById("slcalcod");
	var ofy=document.documentElement ? document.documentElement.scrollTop : document.body.scrollTop;
	var ofx=document.documentElement ? document.documentElement.scrollLeft : document.body.scrollLeft;
...
function checkDate(e)
{
    ...
    else if (checkSpecialKey(null, e))
    {
		return true;
	}
    return false;
...
function checkDateFormat(e, format)
{
    var test=true;
    if((e.value).length>0)
    {
        ...
        if (!test)
        {
            alert("Wrong date format. e.g of correct format is : 10/07/09");
            document.forms[0].elements[e.name].value = "";
            document.forms[0].elements[e.name].focus();
        }
    }
    return test;
}
